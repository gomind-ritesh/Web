...
<html lang="en">
  ...
  ?>
    ...
    <title>Add Food Menu | Red Lantern</title>
...
.back-link:hover {
    color: red; /* Hover color (you can change it to any color you like) */
}
        </style>
        </head>
...
if ($nameErr == "" && $priceErr == "" && $discountErr == "" && $descriptionErr == "" && $categoryErr == "" && $typeErr == "" && $sourceErr == "") {
    ...
    if (move_uploaded_file($file_tmpname, $target_destination)) {
        ...
        if (!$results) {
            ...
            if ($food_id > 0) {
            $update_status = update_food_availability($conn, $food_id);
            if ($update_status) {
                $Msg = "Record saved successfully and previous food marked as unavailable!";
            } else {
                $Msg = "Record saved successfully but failed to mark previous food as unavailable.";
            }
        } else {
...
?>
    ...
    <body>
          ...
          <p>This food item is linked to a bill. Availability has been set to 0. Right now, you are adding a new food since the previous food can't be edited due to lost of analytics</p>
            ...
            <input type="number" class="price" name="price" step="0.01" min="0" value="<?php echo ($priceErr == "" ? $price : '') ?>" required /><br/>
            <span class="error"><?php echo $priceErr; ?></span><br/>

            <label for="discount">Food Discount</label>
            <input type="number" class="discount" name="discount" step="0.01" min="0" value="<?php echo ($discountErr == "" ? $discount : '') ?>" required /><br/>
            <span class="error"><?php echo $discountErr; ?></span><br/>

            <label for="description">Food Description</label>
            <textarea rows="10" cols="10" name="description" class="description" value = "<?php echo ($descriptionErr==""?$description:'')?>" ></textarea>
