...
<div class="card push-top">
    ...
    <div class="card-body">
        ...
        <form method="post" action="{{ route('orders.update', $order->bill_id) }}" id="orderForm">
            @csrf
            @method('PATCH')
            
            <div class="form-group">
                <label for="bill_date">Bill Date</label>
                <input type="date" class="form-control" name="bill_date" value="{{ old('bill_date', $order->bill_date) }}" />
            </div>
            ...
            <div class="form-group">
                ...
                <select name="customer_id" class="form-control">
                    @foreach($customers as $customer)
                        <option value="{{ $customer->customer_id }}" @selected(old('customer_id', $order->customer_id) == $customer->customer_id)>
            ...
            <div class="form-group">
                ...
                <div id="food-items-container">
                    ...
                    @if(old('food_items'))
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="number" min="1" class="form-control quantity-input" 
                                            ...
                                            value="{{ old('food_items')[$index]['quantity'] }}" 
                        ...
                        @endforeach
                    @else
                        @foreach($order->billFoods as $index => $billFood)
                            <div class="food-item">
                                <span class="remove-food" onclick="removeFood(this)">&times;</span>
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <select name="food_items[{{$index}}][food_id]" class="form-control food-select" onchange="updatePrice(this)">
                                            @foreach($foods as $food)
                                                <option value="{{$food->food_id}}" 
                                                    data-price="{{$food->food_price}}" 
                                                    data-discount="{{$food->food_discount}}"
                                                    @selected($billFood->food_id == $food->food_id)>
                                                    {{$food->food_name}} - Rs{{$food->food_price}}
                                                </option>
                                            @endforeach
                                        </select>
            ...
            <div class="form-group">
                ...
                <input type="number" step="0.01" min="0" class="form-control" name="bill_discount" 
                    value="{{ old('bill_discount', $order->bill_discount) }}" id="bill-discount" onchange="updateFinalTotal()"/>
            </div>
            ...
            <div class="form-group">
                ...
                <select name="status" class="form-control">
                    <option value="active" @selected(old('status', $order->status) == 'active')>Active</option>
                    <option value="completed" @selected(old('status', $order->status) == 'completed')>Completed</option>
                    <option value="cancel" @selected(old('status', $order->status) == 'cancel')>Cancel</option>
                </select>
            </div>
                ...
                <div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="reviewed" id="reviewed-yes" value="1"
                            @checked(old('reviewed', $order->reviewed) == '1')>
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" name="reviewed" id="reviewed-no" value="0"
                            @checked(old('reviewed', $order->reviewed) == '0')>
                        <label class="form-check-label" for="reviewed-no">No</label>
                    </div>
                </div>
            </div>
            
            <button type="submit" class="btn btn-block btn-danger">Update Order</button>
        </form>
    </div>
</div>
...
<script>
    ...
    // Calculate the starting index for new food items
    let foodItemIndex = {{ 
        old('food_items') 
            ? count(old('food_items')) 
            : (isset($order) && $order->billFoods ? count($order->billFoods) : 0) 
    }};
